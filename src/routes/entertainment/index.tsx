...
export const Route = createFileRoute("/entertainment/")({
  head: () =>
    pageHead({
      title: "Entertainment",
      description:
        "Pakistani dramas, Bollywood, Pakistani cinema, OTT, reviews and celebrity coverage from SceneBuzz.",
      path: "/entertainment",
...
function EntertainmentHome() {
  const news = articles.filter((a) => a.category !== "Cricket").slice(0, 4);
  return (
    <main className="sb-container-wide py-8">
      ...
      <p className="mt-3 max-w-2xl text-muted">
        Dramas, films, OTT and the people in front of the camera. Sample features are labeled.
      </p>
      <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {[
          { to: "/dramas", label: "Pakistani dramas", img: "/assets/images/hero-drama.jpg" },
          { to: "/movies", label: "Movies", img: "/assets/images/hero-cinema.jpg" },
          { to: "/actors", label: "Actors & celebrities", img: "/assets/images/hero-news.jpg" },
          { to: "/talk-of-the-town", label: "Talk of the Town", img: "/assets/images/hero-fashion.jpg" },
        ].map((x) => (
          <Link key={x.to} to={x.to} className="relative min-h-40 overflow-hidden bg-ink text-paper">
      ...
      <section className="mt-12">
        <SectionHeader title="Dramas" href="/dramas" />
      ...
      <section className="mt-12">
        <SectionHeader title="Upcoming releases" href="/upcoming-movies" />
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {movies.filter((m) => m.upcoming).slice(0, 4).map((m) => (
